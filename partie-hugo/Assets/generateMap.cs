	...
	void Start () {
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < column; j++) {
				...
				if (i < 5 && j >= 5) {
					test [i, j] = "map_medievale_" + Random.Range (4, (nbOfMap + 1));
					while (test [i, j] == (i == 0 ? "" : test [i - 1, j]) || test [i, j] == (j == 0 ? "" : test [i, j - 1]))
							test [i, j] = "map_medievale_" + Random.Range (4, (nbOfMap + 1));
				}
				if (i >= 5 && j < 5) {
		...
		int upgrade1x = Random.Range(0, 5);
		int upgrade1y = Random.Range(0, 5);
		bool upgrade1Placed = false;
		while (!upgrade1Placed) {
			if (test [hpX, hpY] != "map_prehistoire_0" && test [hpX, hpY] != "map_prehistoire_1" && test [hpX, hpY] != "map_prehistoire_2" && test [hpX, hpY] != "map_prehistoire_3") {
				test [hpX, hpY] = "map_prehistoire_3";
				upgrade1Placed = true;
		}

		while (nbHp != 2) {
			int hpX = Random.Range (0, 5);
			int hpY = Random.Range (0.5);
			if (test [hpX, hpY] != "map_prehistoire_0" && test [hpX, hpY] != "map_prehistoire_1" && test [hpX, hpY] != "map_prehistoire_2" && test [hpX, hpY] != "map_prehistoire_3") {
				test [hpX, hpY] = "map_prehistoire_3";
				nbHp++;
			}
